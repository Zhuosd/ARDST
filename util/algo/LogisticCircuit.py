...
from util.algo.LogisticRegression import LogisticRegression
from util.structure.AndGate import AndGate
from util.structure.CircuitNode import CircuitNode, OrGate, CircuitTerminal
from util.structure.CircuitNode import LITERAL_IS_TRUE, LITERAL_IS_FALSE
...
class LogisticCircuit(object):
    ...
    def learn_parameters(self, data, num_iterations, num_cores=-1):
        ...
        # Filling work on disturbances
        train_inf = np.isinf(data.features)
        train_null= np.isnan(data.features)
        data.features[train_inf] = 0
        data.features[train_null] = 0
